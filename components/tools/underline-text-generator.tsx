...
const STORAGE_KEY = "fmc_underline_text_generator";
...
export function UnderlineTextGeneratorTool() {
  ...
  const output = input ? toUnderline(input) : "";
  ...
  const handleExample = () => {
    setInput("Underline text works everywhere!");
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className={cx("rounded-xl border p-4", base)}>
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold">Underlined Output</h3>
            {output && (
              <button
                type="button"
                onClick={copyToClipboard}
          ...
          >
            {output || (
              <span className={cx("text-sm", muted)}>
                U̲n̲d̲e̲r̲l̲i̲n̲e̲d̲ text appears here as you type...
              </span>
            )}
          </div>
          <div className={cx("mt-2 text-xs", muted)}>
      ...
      <div className={cx("rounded-xl border p-4", base)}>
        <p className={cx("text-xs", muted)}>
          <strong className={isDark ? "text-neutral-200" : "text-neutral-700"}>
            Note:
          </strong>{" "}
          Underline rendering varies by platform, browser, and font. The
          combining underline character (U+0332) is a Unicode standard, but some
          fonts may display it differently or with slight gaps between
          characters.
        </p>
      </div>
