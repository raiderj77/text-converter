...
function minifyCss(css: string, preserveComments: boolean): string {
  let src = css;
...
export function CssFormatterTool() {
  ...
  const output = useMemo(() => {
    ...
    return mode === "format"
      ? formatCss(input, indentStyle, preserveComments)
      : minifyCss(input, preserveComments);
  }, [input, indentStyle, mode, preserveComments]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button
          ...
          className={cx(
            "rounded-lg border px-3 py-1.5 text-xs transition-colors min-h-[44px]",
            preserveComments ? btnActive : btnBase
          )}
        >
          {preserveComments ? "Comments: Keep" : "Comments: Remove"}
        </button>
        <button
          onClick={() => {
            setInput("");
      ...
      {output && (
        <div className={cx("rounded-xl border p-4", base)} aria-live="polite">
          ...
          <output
            aria-live="polite"
            className={cx(
              "block rounded-lg border p-3 text-xs font-mono overflow-x-auto whitespace-pre",
              isDark
